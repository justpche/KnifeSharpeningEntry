...
Module GetSharp
    Sub Main()
        ...
        If StrConv(Environment.UserName, VbStrConv.Lowercase) <> "gschell" And StrConv(Environment.UserName, VbStrConv.Lowercase) <> "tvelez" And StrConv(Environment.UserName, VbStrConv.Lowercase) <> "tleece" And StrConv(Environment.UserName, VbStrConv.Lowercase) <> "jims" And StrConv(Environment.UserName, VbStrConv.Lowercase) <> "rmarek" And StrConv(Environment.UserName, VbStrConv.Lowercase) <> "jflores" And StrConv(Environment.UserName, VbStrConv.Lowercase) <> "kvelez" And StrConv(Environment.UserName, VbStrConv.Lowercase) <> "drrivera" And StrConv(Environment.UserName, VbStrConv.Lowercase) <> "marias" And StrConv(Environment.UserName, VbStrConv.Lowercase) <> "jtorres" And StrConv(Environment.UserName, VbStrConv.Lowercase) <> "kburchhart" Then
            Console.ForegroundColor = ConsoleColor.Red
            Console.WriteLine("Hello {0}.  You are not an authorized knife sharpener.", Environment.UserName)
            Console.ReadLine()
            Exit Sub
        End If
        Console.WriteLine("Hello {0}.  This is KnifeSharpeningEntry version 1.6", Environment.UserName)
